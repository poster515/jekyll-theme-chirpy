...
2. Discuss compilation techniques
3. Integrate third party libraries

This blog assumes you have g++ already installed on your computer. Note that this shouldn't be an issue for linix users; however, Windows users will have to download and install this separately, and update their PATH appropriately. This blog will not cover those processes.


## Hello, world!
Ok, so let's jump right in to the code. 
...
```
So hopefully some of these concepts look vaguely familiar. We have an ```#include``` indicating that we want to include a built-in library. We know it's built-in since we're using ```<>``` brackets and not quotations. For non-standard libraries, you'll typically indicate these with quotations (e.g., ```#include "my_lib.hpp"```).

All C++ codes must return an exit code, and must include a ```"main()"``` function. This tells the compiler where the main entry point into the program is. Now in this case, we didn't pass any arguments to the program. We can just as easily do that via:
...
1. Compilation: Actually converting source code into binary, machine-readable code called objects files
...
Note that we simply removed the trailing extension to get this to work on our Linux box. Hopefully it's obvious why this can get out of handle rather quickly. In order to support compilation on multiple machines, it typically behooves us to write Makefiles. We'll talk about that in a second. First, it should be noted that the resulting executable is only executable on machines with your CPU architecture, which is rather unlike languages like Java (which run in virtual machines) and Python (which is ran using an interpreter). 
...
I have used a really neat third party library called Boost many times before. It is really more of a collection of source code written by various people and organizations around the world. There is a lot of useful libraries in there, from multithreading to image processing. Let's say you want to include Boost Generic Image Library (GIL) code, you'll need to know the location of that GIL folder, and edit main.cpp as follows:
